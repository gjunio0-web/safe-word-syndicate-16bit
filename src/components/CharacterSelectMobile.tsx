...
// Every non-font spacing value in the portrait dossier, per tier. Chosen by
// available screen height (see the effect below) so a short, chrome-heavy
// viewport gets the tight set that guarantees no scroll, while a tall or
// truly fullscreen viewport gets real breathing room instead of the same
// starved spacing stretched under a giant portrait. Landscape is untouched
// throughout — every value here only ever lands on a base (unprefixed)
// class, never a `landscape:` one.
...
export const CharacterSelectMobile: React.FC<CharacterSelectMobileProps> = ({ onSelect, onBack }) => {
  ...
  // The spacing tier a device gets is decided purely from available height
  // (not from any content measurement), so it can be picked before the
  // portrait-size measurement below runs — the portrait size depends on
  // how much room the *chosen* spacing leaves, not the other way around.
  // A fullscreen, address-bar-free phone and a browser tab with the
  // address bar showing can be the same device with a >300px difference
  // in usable height; without this, every screen got the same spacing
  // tuned to survive the worst case, which looked starved everywhere else.
  const [tier, setTier] = useState<SpacingTier>('medium');
  const tierRef = useRef<SpacingTier>(tier);
  ...
  useLayoutEffect(() => {
    ...
    const recalc = () => {
      ...
      const nextTier: SpacingTier = available >= 780 ? 'roomy' : available >= 600 ? 'medium' : 'tight';
      if (nextTier !== tierRef.current) {
        // Changing tier changes every padding/gap in the DOM, which this
        // same effect's own portrait-size math depends on — commit that
        // first and let the [tier] dependency below re-run this effect
        // against the new, already-painted spacing, rather than measuring
        // against a mix of old spacing and a new tier.
        setTier(nextTier);
        return;
      }
